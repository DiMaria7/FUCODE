...
# 类别遗忘测试 测试被遗忘类别和保留类别的准确率
def test_class_forget(obj, epoch, model, args, test_loaders):
    forget_acc_ls = []      # 初始化用于存储遗忘类别准确率的列表
    remember_acc_ls = []    # 初始化用于存储记忆类别准确率的列表
    test_result_ls = []     # 初始化用于存储测试结果的列表
    # 遍历每个客户端的测试数据加载器
...
def test_client_forget(obj, epoch, model, args, test_loaders):
    # 生成所有客户端的索引列表（0到num_user-1）
    all_idx = [k for k in range(args.num_user)]
    # 初始化列表：存储遗忘客户端的准确率、保留客户端的准确率、测试结果详情
    forget_acc_ls = []
    remember_acc_ls = []
    test_result_ls = []
    ...
    for k in range(args.num_user):
        # 获取当前客户端的测试数据加载器
        test_loader = test_loaders[k]
        # 构建字典：按类别分组存储测试样本（键为类别标签，值为（数据, 标签）列表）
        label_data_dict = {}
        for data, target in test_loader:
            # 将张量转换为列表，便于按样本拆分
            data = data.tolist()
...
def baizhanting_attack(args, client_loaders, test_loaders):
    ...
    for user in args.forget_client_idx:
        ...
        # 遍历当前客户端的训练数据加载器
        for x, y in client_loaders[user]:
            # 将训练数据图像添加到 dataset_image 列表中
            dataset_image.extend(x)
            # 将训练数据标签添加到 dataset_label 列表中
            dataset_label.extend(y)
        # 遍历当前客户端的测试数据加载器
        for x, y in test_loaders[user]:
            test_image.extend(x)
            test_label.extend(y)
        # 初始化存储修改后的训练数据图像和标签的列表
        data_x = []
        data_y = []
        # 初始化存储修改后的测试数据图像和标签的列表    
        test_x = []
        test_y = []
        # 遍历训练数据标签列表
        for idx, cls in enumerate(dataset_label):
            ...
            # 如果当前标签等于类别总数减 1
                ...
                # 将当前标签设为 0
                dataset_label[idx] = 0
            # 将修改后的训练数据图像添加到 data_x 列表中    
            data_x.append(np.array(dataset_image[idx]))
            # 将修改后的训练数据标签添加到 data_y 列表中
            data_y.append(np.array(dataset_label[idx]))
        # 将 data_x 列表转换为 numpy 数组
        data_x = np.array(data_x)
        data_y = np.array(data_y)
        # 将 data_x 数组转换为 torch.Tensor 类型，并设置为 float32 类型
        data_x = torch.Tensor(data_x).type(torch.float32)
        data_y = torch.Tensor(data_y).type(torch.int64)
        # 创建训练数据列表，每个元素为 (图像, 标签) 元组
        train_data = [(x1, y1) for x1, y1 in zip(data_x, data_y)]
        # 创建新的训练数据加载器，设置批量大小为 local_batch_size，打乱数据
        ...
        for idx, cls in enumerate(test_label):
            ...
            # 如果当前标签等于类别总数减 1
                ...
                # 将当前标签设为 0
                test_label[idx] = 0
            # 将修改后的测试数据图像添加到 test_x 列表中
            test_x.append(np.array(test_image[idx]))
            # 将修改后的测试数据标签添加到 test_y 列表中
...
def membership_inference_attack(args, unlearning_model, case, model, client_all_loaders_bk, test_loaders, proxy_client_loaders_bk, proxy_client_loaders, proxy_test_loaders):
    # 初始化存储测试数据的列表和字典（按用户分组）
    # 全局存储所有测试样本的输出、标签（是否为遗忘数据）、原始类别
    ...
    for i in range(args.num_user):
        ...
        for batch_idx, data in enumerate(data_loader):
            ...
            # 根据遗忘范式标记样本是否为"遗忘数据"（0表示需遗忘，1表示需保留）
            if args.forget_paradigm == 'class':
                # 若为类别遗忘：属于遗忘类别的样本标记为0，其他为1
    ...
    # 处理测试数据（作为成员推断的"非成员"样本
    if args.forget_paradigm == 'class':
        # 类别遗忘：用全局测试集，按类别标记遗忘/保留
    ...
    attack_model.to(args.device)    # 移动到指定设备
    attack_model.train()            # 训练模式
    # 训练攻击模型并获取结果
